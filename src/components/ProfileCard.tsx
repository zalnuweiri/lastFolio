...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  return (
    <div ref={wrapRef} className={`pc-card-wrapper ${className}`.trim()} style={cardStyle}>
      ...
      {/* Mobile-only user info bar - outside the card grid system */}
      {showUserInfo && (
        <div className="pc-user-info-mobile">
          <div className="pc-user-details-mobile">
            <img
              src={finalMiniAvatarUrl}
              className="pc-mini-avatar-mobile"
              alt="Mini Avatar"
              onError={(e) => {
                const target = e.target as HTMLImageElement;
                target.style.opacity = '0.5';
            ...
            />
            <div className="pc-user-text-mobile">
              <div className="pc-handle-mobile">@{handle}</div>
              <div className="pc-status-mobile">{location}</div>
            </div>
          </div>
          <div className="pc-buttons-mobile">
            <button
              className="pc-contact-btn-mobile pc-view-cv-btn-mobile"
              onClick={handleViewCVClick}
              type="button"
              aria-label={`View CV of ${name || 'user'}`}
            >
            ...
            <button
              className="pc-contact-btn-mobile"
              onClick={handleContactClick}
              type="button"
              aria-label={`Contact ${name || 'user'}`}
            >
